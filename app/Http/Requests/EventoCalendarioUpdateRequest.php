...
class EventoCalendarioUpdateRequest extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        return [
            ...
            'tipo' => ['sometimes', 'required', Rule::in(['general', 'formacion', 'retiro_espiritual', 'reunion_equipo'])],
            'alcance' => ['sometimes', 'required', Rule::in(['equipo', 'global'])],
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            // Si se está actualizando las fechas, validar diferencia
            if ($fechaInicio && $fechaFin) {
                ...
                // Si solo se actualiza una fecha, necesitamos el evento actual
                $evento = $this->route('evento');
                if ($evento) {
                    $inicio = $fechaInicio ? \Carbon\Carbon::parse($fechaInicio) : $evento->fecha_inicio;
                    $fin = $fechaFin ? \Carbon\Carbon::parse($fechaFin) : $evento->fecha_fin;
                    $diferenciaAnios = $inicio->diffInYears($fin, false);
            ...
            } elseif ($this->has('hora_inicio') || $this->has('hora_fin')) {
                // Si no es todo el día y se está actualizando horas, ambas son requeridas
